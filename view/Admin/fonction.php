...
function TransfertDocument($nomInput, $nouveauNomFichier, $chemin)
{
...
// Tableaux de donnees
$tabExt = array('pdf','doc','xls','ppt','odt','xlsx','html');    // Extensions autorisees
$infosImg = array();
  ...
  if( !empty($_FILES["$nomInput"]['name']) )
  {
    ...
    if(in_array(strtolower($extension),$tabExt))
    {
      ...
      $infosImg = getimagesize($_FILES["$nomInput"]['tmp_name']);
          ...
          {
            // On renomme le fichier : 
            $nomDoc = $nouveauNomFichier .'.'. $extension;  // uniqid : Génère un identifiant unique, préfixé, basé sur la date et heure courante en microsecondes.
       
            // Si c'est OK, on teste l'upload
            if(move_uploaded_file($_FILES["$nomInput"]['tmp_name'], REPERTOIRE.$nomDoc))
            {
              $message = 'Upload réussi !';
            }
            else
          ...
          {
            $message = 'Une erreur interne a empêché l\'uplaod du document';
          }
     }
    else
    {
